...
.endp
 .else ;EMULATOR==1
midiZPmidiStatus=$BE ;diskZPloadStatus=$F9 ;in KERNAL.asm too
MIDIeventRead=$BF ;$F6 ;in KERNAL.asm too
MIDIeventWritten=$CF ;$F8 ;in KERNAL.asm too
 MIDICMD_GETMIDIDATA=10 ;DISKCMD_GETMIDIDATA=10
 MIDICMD_GOTMIDIDATA=11 ;DISKCMD_GOTMIDIDATA=11
MIDIbuf1=$E009 ;in KERNAL.asm too
        lda #MIDICMD_GETMIDIDATA ;DISKCMD_GETMIDIDATA
        sta MIDIeventRead ;diskZPloadStatus
        ldx #0
        .if (INC_EVENTS!=0)
        ...
        lda #$36
        sta 1
BufLoop lda MIDIeventWritten
        bne BufLoop
        lda MIDIbuf1,x ;$E009,x ;jsr $E003 ;GetMIDIdata ;lda #MIDI_EOD ;...
        ;sta storeBy+1
;storeBy lda #selfmod
        cmp #MIDI_EOD
        beq EndOfDt
...
EndOfDt stx BuffIndex
        ...
        pla
        sta 1
        lda #MIDICMD_GOTMIDIDATA ;DISKCMD_GOTMIDIDATA
        sta MIDIeventRead ;diskZPloadStatus
        ;dec $d020
        rts
 .fi
...
-       sta MIDIbuffer,x
        ...
        pla
        sta 1
        lda #Found_OK      ;tells if successfully initialized
        bcc +
        lda #NotFound
+       sta Status         ;store the response from PC-MIDI
        ;clc ;Carry-flag holds CLK-input on exit (Carry=0 if success)
 .fi
        rts
.pend
...
PowrOFF
.if (EMULATOR==0)
        ldx #160 ;ATN pulsewidth around 800ms for HerMIDI to check it 
