...
MAX_PLAYERSIZE=size(STARTUPMENU.extraPlayer.player) ;take biggest player ;(in the editor player.asm compiles smaller due to the zeropage usage...)
        .union
         ...
         .if (SID_AMOUNT>=2)
          .struct
player2SIDa #playerm PLAYERADDR, nonSID2addr, defaultSID3BASE, defaultSID4BASE ;an address that is not in SID-address range
          .ends
         .fi
         .if (SID_AMOUNT>=3)
          .struct
player3SIDa #playerm PLAYERADDR, defaultSID2BASE, nonSID3addr, defaultSID4BASE
          .ends
         .fi
         .if (SID_AMOUNT>=4)
          .struct
player4SIDa #playerm PLAYERADDR, defaultSID2BASE, defaultSID3BASE, nonSID4addr
          .ends
         .fi
         .struct
player    #playerm PLAYERADDR, defaultSID2BASE, defaultSID3BASE, defaultSID4BASE
         .ends
        .endu
...
.fi
.if (SID_AMOUNT>=4) ;this table will be used to set SID4 address on the fly for the selected player
SID4addTbl .fill size(STARTUPMENU.extraPlayer.SID4ADDtable),0 ;take the biggest ;#SID4ADDm, SID4ADDtable player4SIDa, player, PLAYERADDR  ;SID4-address table
SID4regTbl .fill size(STARTUPMENU.extraPlayer.SID4ADDtable),0 ;the numbers of addressed registers will be copied here in startup-menu
.fi
...
;---------------------FIX pointer-tables-no need to save and recalculate on load-----------------------
        *= SUBTUNES     ;Subtune-orderlist-pointer table (max. 256 bytes, packer will crop unused areas)
        ...
        .if (SID_AMOUNT>1)
         .fill (SID_AMOUNT<3)? (SID_AMOUNT*8-CHN_AMOUNT*2)-2 : ( (SID_AMOUNT<4)? 12 : 6 ) , $00
        .fi
...
leavem2 .fill 2,$00 ;!!!LEAVE EMPTY!!! (placeholder), was used before as config but now they are in 'Config' block (Will be completely removed in SWM2)
 .fi
DrvType .byte 0 ;<(PLAYERID) ;type of the music player/driver routine (0=NORMAL, 1=MID, 2=LIGHT, 3=EXTRA, 4=BARE, 5=DEMO), just an information, no restriction caused
...
ThemeNo .byte 0 ;1 ;colour-theme number
...
PlayerType .byte 0 ;which player (normal/demo/light/extra/bare/demo) to put the cursor on by default in startup-menu
.if (EMULATOR!=0)
confbits2 .byte %00000001 ;bit0: Save dialog automatic disk-directory reading
.else
confbits2 .byte %00000000 ;bit0: Save dialog automatic disk-directory reading
.fi
SID4aID .byte 7 ;number of SID4-address (corresponding addresses can be found in menu.inc)
EOF .byte 0
.bend
Config_end
...
;************************ CHARACTER SETS AND SCREEN DATA **********************
        .cerror (*>$D000),"TOO MUCH MUSICDATA DOESN'T FIT IN MEMORY. DECREASE PATTERNS OR INSTRUMENTS OR SUBTUNES! ",(*-$D000)
;==============================================================================
