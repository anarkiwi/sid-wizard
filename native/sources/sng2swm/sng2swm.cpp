...
{ //TableType: 0:Waveform-table, 1:Pulse-table, 2:Filter-table
 ...
 unsigned char GTvalLeft, GTvalRight, GTvalThird, prevGTvalLeft=0
  , PrevReso=0xF,PrevBand=0x9,PrevCutoff=0xFF, PrevFilterChannelSwitch=0x1, SkippedRowCount=0;
 int GTtableIndex;
 ...
 if ( (GTtableIndex=GTstartIndex) ) //convert GT-table
 {
  ...
  for ( ;GTtableIndex<=0xFF; GTtableIndex++)
  {
   ...
   if ((TableType&0x7F)==0 && GTvalLeft==0x00) GTvalLeft=prevGTvalLeft; //avoid inaudible waveform (when $FE loops)
   if (GTvalLeft==GT2_TABLE_JUMP) {
    if (GTvalRight==0x00) {if (debug) printf(" -> [(%.2X)%.2X: %.2X\n",SWtableIndex, SW1_INSTRUMENT_PARAMSIZE+SWtableIndex, SW1_TABLE_END); break;} //$FF 00 ending
   ...
   else if ((TableType&0x7F)==0 && GTvalLeft>=GT2_WFTABLE_FX) GTvalLeft=GTvalRight=0x00; //no Wavetable-FX support in SW
   else if ((TableType&0x7F)==0) { //WF-table //in case of normal waveform or speed-setting handle arp/pitch values
    if (GTvalLeft >= SW1_TABLE_JUMP /*0xFE*/) GTvalLeft = 0xFD;  //$FE means jump in SID-Wizard, so substitute it by the closest possible value for waveform
   ...
   else if ((TableType&0x7F)==1) { //PW-table
    if (GTvalLeft >= SW1_TABLE_JUMP /*0xFE*/) GTvalLeft = 0xFD;  //$FE means jump in SID-Wizard, so substitute it by the closest possible value for pulsewidth-highbyte setting
   }
   else if ((TableType&0x7F)==2) { //filtertable
    if (GTvalLeft==0x00) {  //convert GT cutoff-setting to SW band+reso+cutoff setting
     GTvalLeft=(PrevBand<<4) | PrevReso;  GTvalThird = 0x80 | PrevFilterChannelSwitch;
     PrevCutoff = GTvalRight;
    }
    else if (GTvalLeft&0x80) { //band/reso/filterswitch setting in GT?
     PrevReso=(GTvalRight>>4); PrevBand=(GTvalLeft&0xF0)>>4; GTvalLeft = (PrevBand<<4) | PrevReso;
      PrevFilterChannelSwitch = (GTvalRight&0xF); GTvalThird = 0x80 | PrevFilterChannelSwitch;
     if (gt2_table->left[(GTtableIndex-1)+1]==0x00) { GTvalRight=gt2_table->right[(GTtableIndex-1)+1]; GTtableIndex++; SkippedRowCount++; } //cutoff-setting in next row? set and skip next row
     else GTvalRight=PrevCutoff; //if no cutoff-setting follows, use previous cutoff
    }
    else { //filter-sweep is only at 8bit-resolution, convert it to SW which sweeps at 11bit resolution
     ...
     GTvalThird = 0x80 | PrevFilterChannelSwitch;
    }
    if (GTvalLeft >= SW1_TABLE_JUMP /*0xFE*/) GTvalLeft = 0xFD;  //$FE/$FF means jump/end in SID-Wizard, so substitute it by the closest possible value for resonance
   }
   if (!(TableType&0x80)) {
...
// Convert Goattracker-header to SID-Wizard header ============================
        ...
        sw_header->colour_theme=0; sw_header->keyboard_type=0; sw_header->driver_type=0; sw_header->tuning_type=0;
        for(i=0; i<sizeof(sw_header->not_defined); ++i) sw_header->not_defined[i] = 0;
