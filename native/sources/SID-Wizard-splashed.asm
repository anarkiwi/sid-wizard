...
.else
 BitmapCol = $CC00       ;target (displayed) address of bitmap-colours (videoRAM)
 Bitmap = $E000          ;target (displayed) address of bitmap including splash-logo
.fi
...
.else
ColMapBuffer=$A000      ;store colours that might be overwritten
.fi
...
.elsif (SID_AMOUNT==4)
        .binary "../application/SID-Wizard-4SID-packed.prg",2
.fi
endpacked
...
SPLASHER lda #$78 ;$A0
         sta $080d      ;restore compress64kSFX launcher's destructed byte
;--------------------------
        sei
        lda #$35
        sta banksel
...
-       lda (sourcePtr),y ;copy character from character ROM
        ...
        lda #$35
        sta banksel
        lda #<dummirq  ;set again IRQ/NMI-vectors possibly overwritten by bitmap-data
        sta $fffa
        lda #>dummirq
        sta $fffb
...
;--------------------------------------------------------
.cerror *>=BitmapCol,"!!! Colour-map area reached. Too much code below !!! ", *-BitmapCol
